...
cat,cocell,colu=catsHTM_parallel.cone_search('FIRST',0,0,500,catalogs_dir=path)
end_time=time.time()
...
for i,j in enumerate(RA):
    catsHTM_parallel.cone_search('FIRST',RA[i],DEC[i],radius,catalogs_dir=path)
end_time=time.time()
